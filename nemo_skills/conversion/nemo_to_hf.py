...
def convert(input_nemo_path, output_hf_path, hf_model_name, max_shard_size, precision=None, cpu_only=False) -> None:
    ...
    weight_shared_name = 'model.module'
    embed_weight = model_state_dict[f'{weight_shared_name}.embedding.word_embeddings.weight']
    ...
    for l in range(int(num_layers)):
        ...
        qkv_weights = model_state_dict[f'{weight_shared_name}.decoder.layers.{l}.self_attention.linear_qkv.weight']
        ...
        # attention dense
        o_weight = model_state_dict[f'{weight_shared_name}.decoder.layers.{l}.self_attention.linear_proj.weight']
        o_weight_base_name = f'model.layers.{l}.self_attn.o_proj.weight'
        checkpoint[o_weight_base_name] = param_to_weights(o_weight)

        # mlp
        mlp_weights = model_state_dict[f'{weight_shared_name}.decoder.layers.{l}.mlp.linear_fc1.weight']
        ...
        mlp_up_proj_weight = model_state_dict[f'{weight_shared_name}.decoder.layers.{l}.mlp.linear_fc2.weight']
        ...
        # layernorm
        input_ln_weight = model_state_dict[f'{weight_shared_name}.decoder.layers.{l}.self_attention.linear_qkv.layer_norm_weight']
        input_ln_base_name = f'model.layers.{l}.input_layernorm.weight'
        checkpoint[input_ln_base_name] = param_to_weights(input_ln_weight)

        post_attn_ln_weight = model_state_dict[f'{weight_shared_name}.decoder.layers.{l}.mlp.linear_fc1.layer_norm_weight']
        post_attn_ln_base_name = f'model.layers.{l}.post_attention_layernorm.weight'
        checkpoint[post_attn_ln_base_name] = param_to_weights(post_attn_ln_weight)

        print(f"done layer {l}")

    final_ln_weight = model_state_dict[f'{weight_shared_name}.decoder.final_layernorm.weight']
    final_ln_base_name = f'model.norm.weight'
    checkpoint[final_ln_base_name] = param_to_weights(final_ln_weight)

    output_layer_weight = model_state_dict[f'{weight_shared_name}.output_layer.weight']
